    ...
    $('.action-account').click(function(){
    	...
    	$('#modal').html(' ');
   		...
   		if (data_username == 'admin' || data_username == 'guest'){
   			...
   			output += '<h5><?php echo lang::get("Create New Usser")?></h5><hr />';
   		}else{
    	...
    	if (data_username != 'admin'){
    		output += '<h5><?php echo lang::get("Home Directory:")?></h5><h6><?php echo lang::get("Isi Dengan Ketentuan Berikut:")?></h6><?php echo lang::get("C:/xampp/htdocs/manajemen/aplikasi/tampilan/univrab/departemen/NAMA DEPARTEMEN/NAMA USER:")?><input type="text" name="homedir" id="homedir" value="'+data_homedir+'" class="text ui-widget-content ui-corner-all" />';
    	...
    	$('#save-button').click(function(){
			...
			$.post("<?php echo gatorconf::get('base_url')?>/?account="+data_username, { is_new: is_new, homedir: homedir, email: email, read: read, write: write, upload: upload, password: set_password} ).done(function(data) {
				// flush 
				window.location.href = '<?php echo "http://localhost/manajemen/index.php/univrab";?>/';
